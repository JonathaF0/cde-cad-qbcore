--[[
    CDECAD Sync - Main Server Script for QBCore
    Handles QBCore events and syncs data to CDECAD
]]
...
-- Build civilian data from QBCore player data
local function BuildCivilianData(source, playerData)
    ...
    -- QBCore uses 'firstname' and 'lastname' (lowercase)
...
local function SyncPlayerVehicles(source, playerData)
    ...
    -- This depends on your vehicle system
...
-- Hook into qb-vehicleshop if you want automatic registration
...
CreateThread(function()
    ...
    print('[CDECAD-SYNC] Using QBCore framework')
    ...
    if Config.Sync.OnCharacterLoad then
        ...
        for _, src in ipairs(players) do
            ...
            if player and player.PlayerData then
                SyncCharacter(src, player.PlayerData, false)
            end
        end
    end
end)

print('[CDECAD-SYNC] Server script loaded (QBCore)')
